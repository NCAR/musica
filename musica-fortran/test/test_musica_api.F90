subroutine test()
  use iso_c_binding
  use micm_core

  implicit none

  type(micm_t)                   :: micm
  real(c_double)                 :: time_step
  real(c_double)                 :: temperature
  real(c_double)                 :: pressure
  integer(c_int)                 :: num_concentrations
  real(c_double), dimension(5)   :: concentrations
  integer                        :: errcode
  character(len=7)  :: config_path

  time_step = 200
  temperature = 272.5
  pressure = 101253.3
  num_concentrations = 5
  concentrations = (/ 0.75, 0.4, 0.8, 0.01, 0.02 /)
  config_path = "chapman"

  micm = micm_t()

  write(*,*) "[test musica] Creating MICM solver..."
  errcode = micm%create_solver(config_path)

  if (errcode /= 0) then
    write(*,*) "[test musica] Failed in creating solver."
    stop 3
  endif

  write(*,*) "[test musica] Initial concentrations", concentrations

  write(*,*) "[test musica] Solving starts..."
  call micm%solve(time_step, temperature, pressure, num_concentrations, concentrations)

  write(*,*) "[test musica] After solving, concentrations", concentrations

  write(*,*) "[test musica] Finished."

end subroutine

program test_musica_api
  call test()
end program