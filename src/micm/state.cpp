...
namespace musica
{
  ...
  {
    std::visit([&](auto& solver_ptr) { this->state_variant_ = solver_ptr->GetState(number_of_grid_cells); }, micm.solver_variant_);
  }

  std::size_t State::NumberOfGridCells()
  {
    return std::visit(
        [](auto& st) -> std::size_t { return st.NumberOfGridCells(); }, state_variant_);
  }
