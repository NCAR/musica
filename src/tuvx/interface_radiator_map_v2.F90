...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  function internal_create_radiator_map(error_code) result(radiator_map) &
    ...
    ! variables
    class(radiative_transfer_t), pointer :: f_radiative_transfer
    class(radiator_warehouse_t), pointer :: f_radiator_warehouse

    f_radiative_transfer => radiative_transfer_t()
    select type(f_radiative_transfer)
    type is(radiative_transfer_t)
      radiator_map = c_loc(f_radiative_transfer%radiator_warehouse_ )
      error_code = 0
    class default
      error_code = 1
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine internal_delete_radiator_map(radiator_map, error_code) &
    ...
    type(radiative_transfer_t), pointer :: f_radiative_transfer
    call c_f_pointer(radiator_map, f_radiative_transfer%radiator_warehouse_)
    deallocate(f_radiative_transfer%radiator_warehouse_)
    error_code = 0
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine internal_add_radiator(radiator_map, radiator, error_code) &
    ...
    error_code = 0
    call f_radiative_transfer%radiator_warehouse_%add(f_radiator)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  function internal_get_radiator(radiator_map, c_radiator_name, & 
    ...
    call c_f_pointer(radiator_map, f_radiative_transfer%radiator_warehouse_)
    write(*,*) "   [jiwon] internal_get_radiator"
    ! This doesn't work
    f_radiator => f_radiative_transfer%radiator_warehouse_%get_radiator(f_radiator_name)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  function internal_get_radiator_updater_from_map(radiator_map, radiator, error_code) &
    ...
    call c_f_pointer(radiator_map, f_radiative_transfer%radiator_warehouse)
    call c_f_pointer(radiator, f_radiator)

    error_code = 0
    allocate(f_updater)
    f_updater = f_radiative_transfer%radiator_warehouse_%get_updater(f_radiator)
