  ...
  py::class_<musica::Radiator>(radiator, "_Radiator")
      ...
      .def_property_readonly(
          ...
          [](musica::Radiator &self)
          {
            ...
            if (!musica::IsSuccess(error))
            {
              std::string message = "Error getting number of wavelength grid sections: " + std::string(error.message_.value_);
              musica::DeleteError(&error);
              throw py::value_error(message);
            }
            musica::DeleteError(&error);
      ...
      .def_property(
          ...
          [](musica::Radiator &self, py::array_t<double, py::array::c_style | py::array::forcecast> array)
          {
            ...
            if (static_cast<size_t>(buf.shape[0]) != num_wavelength_sections || static_cast<size_t>(buf.shape[1]) != num_height_sections)
