...
public:
    ...
    void solve(double time_step, double temperature, double pressure, int num_concentrations, double *concentrations, int num_custom_rate_parameters, double *custom_rate_parameters, Error *error);
...
inline T MICM::get_species_property(const std::string &species_name, const std::string &property_name, Error *error)
{
    ...
    error->category_ = MUSICA_ERROR_CATEGORY;
    std::string msg = "Species '" + species_name + "' not found";
    error->message_ = msg.c_str();
    return T();
}