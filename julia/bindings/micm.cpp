...
//
// MICM bindings for the Julia interface. Registered with the module via the
// self-registration registry (see registration.hpp).
  ...
  void check_index(const Vec& vec, int64_t idx, const char* label)
  {
    if (idx < 0 || static_cast<std::size_t>(idx) >= vec.size())
      throw std::out_of_range(
          std::string(label) + " index " + std::to_string(idx) + " out of range [0, " + std::to_string(vec.size()) + ")");
  }

  void register_micm(jlcxx::Module& mod)
  {
    // ── Version ──────────────────────────────────────────────────────────
    ...
    mod.set_const(
        "SOLVER_STATE_CONVERGENCE_EXCEEDED_MAX_STEPS", static_cast<int>(micm::SolverState::ConvergenceExceededMaxSteps));
    ...
    mod.set_const(
        "SOLVER_STATE_ACCEPTING_UNCONVERGED", static_cast<int>(micm::SolverState::AcceptingUnconvergedIntegration));

    // ── SolverResultStats type ───────────────────────────────────────────
    mod.add_type<musica::SolverResultStats>("CppSolverStats")
        .method(
            "get_function_calls", [](const musica::SolverResultStats& s) { return static_cast<int64_t>(s.function_calls_); })
        .method(
            "get_jacobian_updates",
            [](const musica::SolverResultStats& s) { return static_cast<int64_t>(s.jacobian_updates_); })
        .method(
            "get_number_of_steps",
            [](const musica::SolverResultStats& s) { return static_cast<int64_t>(s.number_of_steps_); })
        ...
        .method(
            "get_decompositions", [](const musica::SolverResultStats& s) { return static_cast<int64_t>(s.decompositions_); })
    ...
    mod.add_type<musica::RosenbrockSolverParameters>("CppRosenbrockParams")
        ...
        .method(
            "get_max_number_of_steps",
            [](const musica::RosenbrockSolverParameters& p) { return static_cast<int64_t>(p.max_number_of_steps); })
        .method(
            "set_max_number_of_steps!",
            [](musica::RosenbrockSolverParameters& p, int64_t v) { p.max_number_of_steps = static_cast<std::size_t>(v); })
        .method(
            "get_absolute_tolerances",
            [](const musica::RosenbrockSolverParameters& p)
            { return std::vector<double>(p.absolute_tolerances.begin(), p.absolute_tolerances.end()); })
        .method(
            "set_absolute_tolerances!",
            [](musica::RosenbrockSolverParameters& p, jlcxx::ArrayRef<double> tols)
    ...
    mod.add_type<musica::BackwardEulerSolverParameters>("CppBackwardEulerParams")
        .constructor<>()
        .method(
            "get_relative_tolerance", [](const musica::BackwardEulerSolverParameters& p) { return p.relative_tolerance; })
        .method(
            "set_relative_tolerance!", [](musica::BackwardEulerSolverParameters& p, double v) { p.relative_tolerance = v; })
        .method(
            "get_max_number_of_steps",
            [](const musica::BackwardEulerSolverParameters& p) { return static_cast<int64_t>(p.max_number_of_steps); })
        .method(
            "set_max_number_of_steps!",
            [](musica::BackwardEulerSolverParameters& p, int64_t v) { p.max_number_of_steps = static_cast<std::size_t>(v); })
        .method(
            "get_absolute_tolerances",
            [](const musica::BackwardEulerSolverParameters& p)
            { return std::vector<double>(p.absolute_tolerances.begin(), p.absolute_tolerances.end()); })
        .method(
            "set_absolute_tolerances!",
            [](musica::BackwardEulerSolverParameters& p, jlcxx::ArrayRef<double> tols)
            { p.absolute_tolerances = std::vector<double>(tols.begin(), tols.end()); })
        .method(
            "get_time_step_reductions",
            [](const musica::BackwardEulerSolverParameters& p)
            { return std::vector<double>(p.time_step_reductions.begin(), p.time_step_reductions.end()); })
        .method(
            "set_time_step_reductions!",
            [](musica::BackwardEulerSolverParameters& p, jlcxx::ArrayRef<double> vals)
    ...
    mod.method(
        "cpp_create_solver",
        [](const std::string& config_path, int solver_type)
        {
          musica::Error error;
    ...
    mod.method(
        ...
        {
          musica::Error error;
          musica::MICM* micm = musica::CreateMicmFromConfigString(
              config_string.c_str(), static_cast<musica::MICMSolver>(solver_type), &error);
          if (!musica::IsSuccess(error))
          {
            std::string msg = "Error creating solver: " + std::string(error.message_.value_);
            musica::DeleteError(&error);
    ...
    mod.method(
        "cpp_create_state",
        [](musica::MICM* micm, int64_t number_of_grid_cells)
        {
          check_micm_not_null(micm);
          musica::Error error;
          musica::State* state = musica::CreateMicmState(micm, static_cast<std::size_t>(number_of_grid_cells), &error);
          if (!musica::IsSuccess(error))
          {
    ...
    mod.method(
        "cpp_state_set_concentration!",
        [](musica::State* state, int64_t idx, double value)
        {
          check_state_not_null(state);
          auto& concs = state->GetOrderedConcentrations();
          check_index(concs, idx, "Concentration");
    ...
    mod.method(
        "cpp_state_get_condition_temperature",
        [](musica::State* state, int64_t idx)
        {
          check_state_not_null(state);
    ...
    mod.method(
        "cpp_state_get_condition_pressure",
        [](musica::State* state, int64_t idx)
        {
          check_state_not_null(state);
          auto& conds = state->GetConditions();
    ...
    mod.method(
        "cpp_state_get_condition_air_density",
        [](musica::State* state, int64_t idx)
        {
          check_state_not_null(state);
          auto& conds = state->GetConditions();
    ...
    mod.method(
        "cpp_state_species_ordering_names",
        [](musica::State* state)
        {
          check_state_not_null(state);
    ...
    mod.method(
        "cpp_state_species_ordering_indices",
        [](musica::State* state)
        {
          check_state_not_null(state);
          auto map = state->GetVariableMap();
    ...
    mod.method(
        "cpp_state_rate_param_ordering_names",
        [](musica::State* state)
        {
          check_state_not_null(state);
    ...
    mod.method(
        "cpp_state_rate_param_ordering_indices",
        [](musica::State* state)
        {
          check_state_not_null(state);
          auto map = state->GetRateParameterMap();
    ...
    // ── Solver parameters ───────────────────────────────────────────────
    mod.method(
        "cpp_set_rosenbrock_params",
        [](musica::MICM* micm, const musica::RosenbrockSolverParameters& params)
        {
          check_micm_not_null(micm);
    ...
    // ── CUDA availability ───────────────────────────────────────────────
    mod.method("cpp_is_cuda_available", []() { return musica::IsCudaAvailable(); });
  }

  [[maybe_unused]] const musica_julia::Registrar registrar(register_micm);
}  // namespace
