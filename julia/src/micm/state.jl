...
function set_concentrations!(state::State, concentrations::Dict{String,<:Any})
    ...
    for (name, value) in concentrations
        haskey(state._species_ordering, name) ||
            error("Species $name not found in the mechanism.")
        ...
        for i_cell = 1:state._number_of_grid_cells
...
function set_user_defined_rate_parameters!(state::State, params::Dict{String,<:Any})
    ...
    for (name, value) in params
        haskey(state._rate_parameter_ordering, name) ||
            error("User-defined rate parameter $name not found in the mechanism.")
...
function set_conditions!(
    ...
    for i_cell = 1:n
        ...
        cur_temp =
            temps !== nothing ? temps[i_cell] :
            cpp_state_get_condition_temperature(state._ptr, idx)
        cur_pres =
            press !== nothing ? press[i_cell] :
            cpp_state_get_condition_pressure(state._ptr, idx)
        if dens !== nothing
