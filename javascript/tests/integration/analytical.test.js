...
function testAnalytical(solver, state, numCells = 1, step = 1, places = 5) {
    ...
    for (let i = 0; i < numCells; i++) {
        ...
        press.push(pressure);
        conc.A.push(0.75); conc.B.push(0); conc.C.push(0.4);
        conc.D.push(0.8); conc.E.push(0); conc.F.push(0.1);
    ...
    for (let t = step; t <= 100; t += step) {
        ...
        for (let i = 0; i < numCells; i++) {
            ...
            const k4 = 0.012 * Math.exp(75.0 / temps[i]) *
                       Math.pow(temps[i] / 50.0, -2) * (1.0 + 1.0e-6 * press[i]);

            const A = conc.A[i] * Math.exp(-k3 * t);
            const B = conc.A[i] * (k3 / (k4 - k3)) * (Math.exp(-k3 * t) - Math.exp(-k4 * t));
            const C = conc.C[i] + conc.A[i] * (1.0 + (k3 * Math.exp(-k4 * t) - k4 * Math.exp(-k3 * t)) / (k4 - k3));
            const D = conc.D[i] * Math.exp(-k1 * t);
            const E = conc.D[i] * (k1 / (k2 - k1)) * (Math.exp(-k1 * t) - Math.exp(-k2 * t));
            const F = conc.F[i] + conc.D[i] * (1.0 + (k1 * Math.exp(-k2 * t) - k2 * Math.exp(-k1 * t)) / (k2 - k1));
...
const solvers = [
  ...
  { name: 'Rosenbrock Standard', type: SolverType.rosenbrock_standard_order, step: 200.0, places: 5 },
  { name: 'Backward Euler', type: SolverType.backward_euler, step: 10.0, places: 2 },
  { name: 'Backward Euler Standard', type: SolverType.backward_euler_standard_order, step: 10.0, places: 2 },
];
