...
StateWrapper::StateWrapper(musica::State* state)
    : state_(state), deconstruct(true) {
    // Always cleanup by default - External finalizer will delete this wrapper
}
...
void StateWrapper::DeleteStateWrapper() {
    // This method is deprecated - cleanup happens automatically in destructor
    // Keeping for API compatibility but it's a no-op now
}
...
void StateWrapper::SetConcentrations(const std::map<std::string, std::vector<double>>& concentrations) {
    ...
    musica::GetSpeciesOrdering(state_, &species_ordering, &error);
    ...
    size_t array_size;
    double* conc_ptr = musica::GetOrderedConcentrationsPointer(state_, &array_size, &error);
    if (!musica::IsSuccess(error)) {
    ...
    musica::GetConcentrationsStrides(state_, &error, &cell_stride, &species_stride);

    size_t num_cells = musica::GetNumberOfGridCells(state_, &error);
...
void StateWrapper::SetUserDefinedRateParameters(const std::map<std::string, std::vector<double>>& params) {
    ...
    musica::GetUserDefinedRateParametersOrdering(state_, &params_ordering, &error);
    if (!musica::IsSuccess(error)) {
        musica::DeleteError(&error);
    ...
    size_t array_size;
    double* params_ptr = musica::GetOrderedRateParametersPointer(state_, &array_size, &error);
    if (!musica::IsSuccess(error)) {
    ...
    musica::GetUserDefinedRateParametersStrides(state_, &error, &cell_stride, &param_stride);
...
void StateWrapper::SetConditions(const std::vector<double>* temperatures,
                                 const std::vector<double>* pressures,
                                 const std::vector<double>* air_densities) {
    musica::Error error;
    size_t array_size;
    micm::Conditions* conditions = musica::GetConditionsPointer(state_, &array_size, &error);
...
double* StateWrapper::GetConcentrationsPointer(size_t& array_size) {
    musica::Error error;
    double* ptr = musica::GetOrderedConcentrationsPointer(state_, &array_size, &error);
    musica::DeleteError(&error);
    return ptr;
}
