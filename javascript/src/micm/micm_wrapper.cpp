...
// Private constructor for pre-created MICM
MICMWrapper::MICMWrapper(musica::MICM* micm, int solver_type)
    : micm_(micm), solver_type_(solver_type) {
}

// Factory method to create from JSON/YAML string
MICMWrapper* MICMWrapper::CreateFromConfigString(const std::string& config_string, int solver_type) {
    musica::Error error;
    ...
    return new MICMWrapper(micm, solver_type);
}
