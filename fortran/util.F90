...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   subroutine assert( condition, file, line, error_message )
      ...
      write(line_str, '(I0)') line  ! Convert integer line to a string

      if ( .not. condition ) then
         if ( present( error_message ) ) then
            write(*,*) "[MUSICA ERROR at ", trim(file)//":"//trim(line_str), "] ", error_message
         else
            write(*,*) "[MUSICA ERROR at ", trim(file)//":"//trim(line_str), "] Assertion failed"
         end if
         stop 3
      end if
