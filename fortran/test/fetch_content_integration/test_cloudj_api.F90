...
!
program combined_cloudj_tests
  use iso_c_binding
  use musica_cloudj, only: cloudj_t, grid_map_t, grid_t, profile_map_t, profile_t, &
...
contains
  ...
  subroutine test_cloudj_api()
    ...
    ASSERT( error%is_success() )
    cloudj => cloudj_t(config_path, grids, profiles, radiators, error)
    ASSERT( error%is_success() )
  ...
  end subroutine test_cloudj_api

  ! Invalid cloudj solver creation test
  subroutine test_cloudj_api_invalid_config()
    character(len=256)            :: config_path
    type(grid_map_t),     pointer :: grids
    type(profile_map_t),  pointer :: profiles
    type(radiator_map_t), pointer :: radiators
  ...
  end subroutine test_cloudj_api_invalid_config

  subroutine test_cloudj_solve()
    ...
    ASSERT( error%is_success() )
    cloudj => cloudj_t(config_path, grids_from_host, profiles_from_host, radiators_from_host, error)
    ASSERT( error%is_success() )

    grids => cloudj%get_grids( error )
    ASSERT( error%is_success() )
    ...
    profiles => cloudj%get_profiles( error )
    ASSERT( error%is_success() )
    ...
    radiators => cloudj%get_radiators( error )
    ASSERT( error%is_success() )
...
!
  ...
  end subroutine test_cloudj_solve

end program combined_cloudj_tests
