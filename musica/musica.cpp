...
void bind_musica(py::module_ &core)
{
  ...
  core.def(
      ...
      {
        return HandleMusicaErrors([&](musica::Error *error) { return musica::CreateMicm(config_path, solver_type, error); });
      });
  ...
  core.def(
      ...
      {
        return HandleMusicaErrors(
            [&](musica::Error *error)
            {
              musica::Chemistry chemistry = musica::ConvertV1Mechanism(mechanism);
              return musica::CreateMicmFromChemistryMechanism(&chemistry, solver_type, error);
            });
      });
  ...
  core.def(
      ...
      {
        return HandleMusicaErrors([&](musica::Error *error)
                                  { return musica::CreateMicmState(micm, number_of_grid_cells, error); });
      });
