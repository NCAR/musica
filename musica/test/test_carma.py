...
def test_carma_aluminum():
    ...
    mmr = 5e9 / (params.deltaz * 2.57474699e14) / env["air_density"][0]
    for i in range(len(params.elements)):
        for j in range(len(params.wavelength_bins)):
            state.set_bin(i + 1, j + 1, mmr)
    print()
    ...
    for step in range(1, int(params.nstep)):
        ...
        for i in range(len(params.elements)):
            for j in range(len(params.wavelength_bins)):
                print(f"Step {step}, Bin {i + 1}, Wavelength {j + 1}: {state.get_bin(i + 1, j + 1)}")
